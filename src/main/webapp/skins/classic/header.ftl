<div class="nav">
    ...
    <div class="nav-tabs">
        ...
        <a pjax-title="${domain.domainTitle} - ${domainLabel} - ${symphonyLabel}" href="${servePath}/domain/${domain.domainURI}"<#if selected?? && selected == domain.domainURI> class="current"</#if>>${domain.domainIconPath} ${domain.domainTitle}
        ...
        <#--<a href="${servePath}/tag/book_share"<#if selected?? && 'book' == selected> class="current"</#if>>-->
    ...
    <form class="responsive-hide fn-left" target="_blank" action="/search">
        <input class="search" placeholder="Search you want in AIQ" type="text" name="key" id="search" value="<#if key??>${key}</#if>" >
